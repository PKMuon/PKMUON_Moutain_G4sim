...
void analysis(const char* path, const char* oldroot, const char* newroot){
TRandom *rand = new TRandom();
double sigma=100*0.001;

TFile *f = new TFile(Form("%s/root/%s",path,oldroot));
TTree *t = (TTree*) f->Get("T1");

//TChain *t = new TChain("T1");
//t->Add(Form("%s/root/*root",path));
...
TFile * fn = new TFile(Form("%s/newroot/%s",path,newroot),"recreate");
